...
PeatLink is a decentralized chat platform built for environments where infrastructure can't be relied on. Peers communicate directly over [iroh](https://iroh.computer) gossip, WiFi Direct P2P, or Bluetooth LE mesh -- with automatic multi-hop relay across transport boundaries. The app works fully standalone without any server: voice, chat, CoT/GPS, markers, and DMs all function between peer-to-peer devices. When connectivity returns, everything syncs upstream automatically. Messages are stored locally using [Automerge](https://automerge.org) CRDTs for conflict-free sync.
...
**`peatlink-mobile`** -- Embeds a **full-featured axum WebSocket server** inside mobile apps that handles all features locally -- chat, voice signaling, CoT/GPS, markers, DMs, edits, reactions -- without needing a Go server. When upstream connectivity exists, all operations are transparently relayed. React UI connects to `localhost:{port}` via WebView. Native bindings generated by UniFFI (Kotlin + Swift). BLE mesh via `peat-btle` with a full BLE-to-WS bridge (`bridge.rs`) that tunnels all message types over BLE using `__ws:`-prefixed JSON envelopes. WiFi Direct P2P via `PeatWifiDirectService.kt` for medium-range offline transport.

**Go Server** -- WebSocket relay for the web UI. Manages rooms (public/private), Ed25519 client sessions, message history (1000 per room), mesh topology broadcasts with transport priority dedup, **WebRTC voice signaling relay**, **CoT position/marker brokering**, **room discovery broadcasting**, and **BLE/WiFi Direct peer registration**.

**Web UI** -- React 18 + Zustand + Tailwind CSS. Dark-themed chat interface with room sidebar, **room discovery (public rooms)**, **room creation (public/private toggle)**, message threads, unread badges, SVG mesh topology viewer, **push-to-talk voice channels** (PTT/Noise Gate/Auto modes), **tactical map (MapLibre GL)**, and **settings page**.

**Mobile Shells** -- Kotlin (Android) and Swift (iOS) wrappers that load the React UI in a WebView, handle BLE/WiFi Direct/microphone permissions, optional **foreground service for background mesh operation**, and interface with the embedded Rust server via UniFFI.
...
| **Network** | Preferred transport (TCP, WiFi Direct, BLE), background mesh toggle |
...
│   │   └── app/src/main/kotlin/
│   │       ├── MainActivity.kt   #   WebView + UniFFI + service binding
│   │       ├── PeatMeshService.kt #  foreground service for background mesh
...
```
 ...
 ------------------                ------------------
  ...
  create_room {name, is_public}    room_list {rooms[]}
  list_rooms                       room_history {room_id, messages[]}
  send_message {room_id,           message {room_id, message}
    content, reply_to?}            peer_update {room_id, peer_id, event}
  set_transport {transport}        mesh_state {room_id, self_id, peers[]}
  set_preferred_transport          ble_mesh_state {room_id, peers[]}
    {transport}                    error {message}
  register_ble_peer
    {peer_id, peer_name,
     transport?}
  start_dm {target_id}             dm_opened {room_id, peer_id, peer_name}
  join_dm {room_id}
```
...
**Standalone operation** -- The embedded Rust server in peatlink-mobile handles all features locally: chat, voice signaling (WebRTC offer/answer/ICE relay), CoT/GPS, markers, DMs, edits, reactions, and pins. Two phones on WiFi Direct or BLE can use every feature without a Go server. All handlers also forward to `passthrough_tx` so operations sync upstream when connectivity returns.

**Room management** -- Rooms support public (discoverable by all mesh clients) and private (join by exact name) modes. The Go server broadcasts a `room_list` of public rooms when rooms change. The embedded server forwards `join_room` upstream so rooms sync across the mesh. Room IDs are deterministic (`blake3(name)`) — identical on both Go and Rust servers.

**Voice channels** use WebRTC for peer-to-peer audio. Both the Go server and embedded Rust server relay voice signaling (SDP offers/answers, ICE candidates). Three voice modes: PTT, Noise Gate (energy-based dB threshold), and Auto (WebRTC VAD). Mode switching cleanly tears down the old audio pipeline. Listen-only fallback when no mic is available.

**CoT positions** are broadcast to all room members and stored per-room. Both the Go server and embedded server maintain CoT state independently. Markers carry full CoT metadata (type, how, ce, le, hae, stale, remarks) for ATAK plugin interoperability. BLE peer positions are injected into the relay phone's room state so all clients see them.

**Direct messages** work both online (Go server mediates via `clientsByID`) and offline (embedded server maintains `known_peers` registry from mesh state, creates deterministic DM rooms). DMs are private — the bridge filters `dm_opened` and DM room history from BLE forwarding.

**BLE mesh** -- On Android, `PeatBleService.kt` provides dual-role BLE (scan + advertise + GATT) with dual priority queues (audio cap 20, data cap 100) and 20ms audio+bridge drain. `bridge.rs` tunnels full WS JSON envelopes over BLE using a `__ws:` prefix protocol and generates server-style events locally (voice_peer_joined, cot_state) so BLE peer actions are visible without upstream. `BleVoiceService.kt` adds Opus 8kbps voice relay (~1KB/s per speaker) with 60ms jitter buffer. BLE peers are periodically re-registered upstream to handle startup race conditions. MAC→callsign promotion happens via `ble_hello` handshake within ~40ms.

**WiFi Direct** -- `PeatWifiDirectService.kt` handles Android WiFi Direct P2P discovery (DNS-SD `_peatlink._tcp`), group formation with automatic GO election (internet-connected device wins), and retry loop for channel initialization. WiFi Direct clients connect to the GO's embedded WS server and start an upstream relay with transport label `"wifi-direct"`. Multi-hop chains (Go Server → WiFi Relay → WiFi Direct Phone → BLE Phone) work transparently.

**Transport priority** -- The server deduplicates peers reachable via multiple transports, preferring TCP > WiFi Direct > BLE by default. Users can override via Settings → Preferred Transport. Implemented in `hub.go` via `transportPriority()` and `deduplicatePeers()`.

**Background mesh** -- `PeatMeshService.kt` is an Android foreground service that keeps BLE, WiFi Direct, and the Rust server alive when the app is backgrounded or the screen is off. Toggle in Settings. Uses `FOREGROUND_SERVICE_TYPE_CONNECTED_DEVICE` with a persistent notification showing mesh status. When disabled (default), the app behaves normally and tears down on exit.
...
- **Standalone operation** -- The embedded server handles ALL features locally (chat, voice signaling, CoT/GPS, markers, DMs, edits, reactions, pins) without needing a Go server. Two phones on WiFi Direct or BLE can do everything. When upstream connectivity exists, all operations are transparently relayed.
- **Upstream WS relay** -- connects to a Go server on the LAN, bidirectionally bridges all message types with echo suppression. Also re-registers BLE peers periodically to handle startup race conditions.
- **WiFi Direct P2P** -- `PeatWifiDirectService.kt` discovers peers via DNS-SD (`_peatlink._tcp`), forms P2P groups, and auto-connects upstream relay to Group Owner's embedded WS server. Retry loop for channel init when WiFi isn't immediately ready.
- **BLE mesh** -- `PeatBleService.kt` runs a dual-role BLE driver (scanner + advertiser + GATT server/client). `bridge.rs` tunnels all WS message types over BLE using `__ws:`-prefixed JSON envelopes. Bridge generates server-style events locally (voice_peer_joined, cot_state) so BLE peer actions are visible without upstream. DM privacy filtering prevents private conversations from leaking to BLE peers.
- **BLE voice relay** -- `BleVoiceService.kt` captures audio via `AudioRecord`, encodes to Opus (8kHz mono, 8kbps, 20ms frames) via `MediaCodec`. 20ms audio drain loop with dual priority queues (audio cap 20, data cap 100). `WRITE_TYPE_NO_RESPONSE` for audio frames. 60ms jitter buffer on playback.
- **Background mesh** -- `PeatMeshService.kt` foreground service keeps BLE/WiFi Direct/Rust server alive when the app is backgrounded or screen is off. Persistent notification shows mesh status. Toggle in Settings.
- **Room discovery** -- Public rooms are broadcast to all clients. Create rooms with public/private toggle. Sidebar shows joinable rooms.
